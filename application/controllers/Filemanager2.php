...
class Filemanager2 extends CI_Controller {
	...
	public function index() {
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href' => site_url('filemanager2') . '?directory=' . substr($image, strlen(DIR_IMAGE . 'catalog/')) . $url,
				);
		...
		$data['parent'] = site_url('filemanager2') . '?token=token' . $url;
		...
		$data['refresh'] = site_url('filemanager2') . '?token=token' . $url;
		...
		//$pagination->limit = 16;
		//$pagination->url = site_url('filemanager2').'?token=token'. $url . '&page={page}';
		//$data['pagination'] = $pagination->render();

		$config['base_url'] = site_url('filemanager2');
		$config['total_rows'] = $image_total;
		$config['per_page'] = 16;
		$config['page'] = $page;
		$config['url'] = $url;
		$data['pagination'] = $this->pagination($config);
		$data['c'] = site_url('filemanager2');
		$this->load->view('filemanager2', $data);
	}
